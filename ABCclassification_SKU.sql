...
,df3 AS
	(SELECT	SKUs_new,
			SUM(Order_No_New)*100/ (SELECT SUM(Order_No_New) FROM df2) percentage_SKU_ord
	FROM		df2
	GROUP BY	SKUs_new
	)

SELECT	*,
		CASE WHEN percentage_SKU_ord < 80 THEN 'A' 
			WHEN percentage_SKU_ord >= 80 AND percentage_SKU_ord <= 95 THEN 'B' 
			WHEN percentage_SKU_ord > 95 THEN 'C' 
		ELSE 'N/A'
		END AS SKU_classification 
FROM	df3
ORDER BY	percentage_SKU_ord DESC;
