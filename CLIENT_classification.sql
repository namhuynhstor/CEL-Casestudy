...
,df3 AS
	(SELECT	Ship_To_Address_No_New,
			SUM(Order_No_New)*100/ (SELECT SUM(Order_No_New) FROM df2) percentage_CLIENT_ord
	FROM		df2
	GROUP BY	Ship_To_Address_No_New
	)

SELECT	*,
		CASE WHEN percentage_CLIENT_ord < 80 THEN 'A' 
			WHEN percentage_CLIENT_ord >= 80 AND percentage_CLIENT_ord <= 95 THEN 'B' 
			WHEN percentage_CLIENT_ord > 95 THEN 'C' 
		ELSE 'N/A'
		END AS CLIENT_classification 
FROM	df3
ORDER BY	percentage_CLIENT_ord DESC;
